...
var _deleteInWithCleanUp = _interopRequireDefault(
...
var isReduxFormAction = function isReduxFormAction(action) {
  return (
    ...
    action.type.substring(0, _actionTypes.prefix.length) === _actionTypes.prefix
  )
}
...
function createReducer(structure) {
  ...
  var plainDeleteInWithCleanUp = (0, _deleteInWithCleanUp['default'])(
    _plain['default']
  )(shouldDelete)
  ...
  var doPlainSplice = function doPlainSplice(
    ...
    return existing || force
      ? setIn(
          state,
          key,
          _plain['default'].setIn(
            slice,
            field,
            _plain['default'].splice(existing, index, removeNum, value)
          )
        )
      : state
  }
  ...
  (_behaviors[_actionTypes.ARRAY_INSERT] = function(state, _ref2) {
  ...
  (_behaviors[_actionTypes.CLEAR_FIELDS] = function(state, _ref16) {
    ...
    var anyTouched = some(keys(getIn(result, 'registeredFields')), function(
      key
    ) {
  ...
  (_behaviors[_actionTypes.STOP_ASYNC_VALIDATION] = function(state, _ref22) {
    ...
    if (payload && Object.keys(payload).length) {
      var _error = payload._error,
        fieldErrors = (0, _objectWithoutPropertiesLoose2['default'])(payload, [
          '_error'
        ])
  ...
  (_behaviors[_actionTypes.STOP_SUBMIT] = function(state, _ref23) {
    ...
    var isError =
      (0, _isObject2['default'])(payload) && Object.keys(payload).length
    ...
    if (isError || isFailure) {
      ...
      if (isError) {
        var _error = payload._error,
          fieldErrors = (0, _objectWithoutPropertiesLoose2['default'])(
            payload,
            ['_error']
          )
        result = deleteIn(result, 'failure')
  ...
  (_behaviors[_actionTypes.UNREGISTER_FIELD] = function(state, _ref26) {
    ...
    if (count <= 0 && destroyOnUnmount) {
      ...
      if (syncWarnings) {
        ...
        if (
          _plain['default'].deepEqual(syncWarnings, _plain['default'].empty)
        ) {
  ...
  var byForm = function byForm(reducer) {
    return function(state, action) {
      ...
      if (
        action.type === _actionTypes.DESTROY &&
        action.meta &&
        action.meta.form
      ) {
